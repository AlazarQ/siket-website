...
class CSRController extends Controller
{
    public function index(Request $request){
        ...
        $csrs = DB::select("with onlyEnglish as (
            SELECT * FROM csr b where b.language = 'en' and b.status = 'Active' order by b.id ASC
             ),
             onlyLanguage as (
    ...
    public function detail($title){
        ...
        $relatedCSR =DB::select("with onlyEnglish as (
            SELECT * FROM csr b where b.id !='$id' and b.language = 'en' and b.status = 'Active' order by b.id ASC LIMIT 3
             ),
             onlyLanguage as (
             SELECT ol.* FROM onlyEnglish ol where  ol.id not in (select b.parent_id from csr b  where b.language = '$lang_code')
             union
