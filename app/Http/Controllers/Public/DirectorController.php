...
class DirectorController extends Controller
{
    public function index($id){
        try{
            ...
            $chief = BoardDirector::where('id',$id)->first();
            $directors = DB::select("with onlyEnglish as (
            SELECT * FROM board_director b where b.type='director' and b.cheif_id = '$id' and  b.language = 'en' and b.status = 'Active' order by b.id ASC
             ),
             onlyLanguage as (
             SELECT ol.* FROM onlyEnglish ol where  ol.id not in (select b.parent_id from board_director b  where b.language = '$lang_code')
             union
             select * from board_director bn where bn.type='director' and bn.cheif_id = '$id' and bn.language = '$lang_code' and bn.status = 'Active'
